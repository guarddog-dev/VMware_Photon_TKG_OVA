...
build {
  ...
  provisioner "shell" {
    execute_command = "echo ${var.guest_password} | sudo -S bash -c '{{ .Path }}'"
    script          = "scripts/4-installvmwcli.sh"
  }

  provisioner "shell" {
    environment_vars = ["TKGVERSION=${var.version}","VMWUSER=${var.vmwuser}","VMWPASS=${var.vmwpass}"]
    execute_command = "echo ${var.guest_password} | sudo -S bash -c '{{.Vars}} {{ .Path }}'"
    script          = "scripts/5-installvmwaretkgcli.sh"
  }

  provisioner "shell" {
    execute_command = "echo ${var.guest_password} | sudo -S bash -c '{{ .Path }}'"
    script          = "scripts/6-installkubectl.sh"
  }
